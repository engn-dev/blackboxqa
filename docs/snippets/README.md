...
content between `<!-- blackboxqa:snippet … -->` markers or the generated TypeScript module.
...
- `{{cli}}` is substituted per consumer: `blackboxqa` in CLI help (the default), `npx @blackboxqa/cli`
...
- Facts must match the daemon implementation (`apps/blackboxqa-daemon/src/`) — e.g.
