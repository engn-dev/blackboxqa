...
Thanks for your interest in blackboxqa.
...
```bash
pnpm --filter @blackboxqa/daemon  dev   # daemon in watch mode
pnpm --filter @blackboxqa/browser dev   # CLI in watch mode
pnpm --filter @blackboxqa/cli     dev   # blackboxqa in watch mode
```

## House rules

- No `console.*` in committed code — use `@blackboxqa/logger` for diagnostics and `process.stdout` for CLI output (enforced by Biome's `noConsole`).
- All new code is TypeScript with `strict: true` and no `any`.
- Tests are vitest; colocated for daemon, in `test/` for cli + blackboxqa.
