...
export interface CreateLoggerOptions {
  ...
  /** Level used when neither `level` nor `BBQA_LOG_LEVEL` is set. Default: "info". */
  fallbackLevel?: LogLevel;
  /** Explicit level. Overrides the `BBQA_LOG_LEVEL` environment variable. */
  level?: LogLevel;
  /** Base `name` binding attached to every record (e.g. "daemon", "blackboxqa"). */
  name?: string;
...
/** Resolve the effective log level from options then `BBQA_LOG_LEVEL` then fallback. */
export function resolveLevel(opts: CreateLoggerOptions = {}): LogLevel {
  if (opts.level) {
    return opts.level;
  }
  const fromEnv = process.env.BBQA_LOG_LEVEL?.trim().toLowerCase();
