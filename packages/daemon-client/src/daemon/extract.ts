...
// template into ~/.blackboxqa/ if missing or stale. Returns the daemon bundle path.
export async function ensureDaemonExtracted(): Promise<string> {
  const dir = blackboxqaDir();
...
// `blackboxqa install` has been run). The set checked is derived from the single
