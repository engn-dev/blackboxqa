...
async function commandFromEntry(entry: string): Promise<DaemonCommand> {
  ...
  try {
    ...
    throw new Error(
      `Failed to resolve BBQA_DAEMON entry ${abs}: ${(err as Error).message}`
    );
  }
