...
describe("paths", () => {
  const home = homedir();
  const base = join(home, ".blackboxqa");

  it("anchors everything under ~/.blackboxqa", () => {
    expect(blackboxqaDir()).toBe(base);
  ...
  it("daemonEndpoint picks platform-specific transport", () => {
    ...
    if (process.platform === "win32") {
      expect(endpoint.startsWith("\\\\.\\pipe\\blackboxqa-daemon-")).toBe(true);
    } else {
      expect(endpoint).toBe(daemonSocketPath());
    }
  });

  it("derives the session artifact layout under ~/.blackboxqa/sessions/<id>", () => {
