...
describe("ensureDaemonExtracted", () => {
  it("writes daemon.mjs, sandbox-client.js, and package.json under ~/.blackboxqa", async () => {
    const daemonPath = await ensureDaemonExtracted();
    expect(daemonPath).toBe(join(tempHome, ".blackboxqa", "daemon.mjs"));
    for (const name of ["daemon.mjs", "sandbox-client.js", "package.json"]) {
      const info = await stat(join(tempHome, ".blackboxqa", name));
  ...
  it("skips rewrite when content matches", async () => {
    ...
    const bundlePath = join(tempHome, ".blackboxqa", "daemon.mjs");
  ...
  it("rewrites when content is stale", async () => {
    const dir = join(tempHome, ".blackboxqa");
