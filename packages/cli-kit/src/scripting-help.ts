// Shared scripting-help prose for the `blackboxqa` and `blackboxqa-browser` CLIs.
...
// `blackboxqa-browser` vs `blackboxqa run --session … --step …`).
import {
  API_BROWSER,
  API_CONSOLE,
  API_FILE_HELPERS,
...
export interface ExampleOptions {
  ...
  connect?: boolean;
  // Step label for session-style invocations (`blackboxqa run … --step <step>`).
  step?: string;
}
...
// blackboxqa-browser: heredoc straight to the engine; `--connect` when the example
// targets an already-running Chrome.
export const browserExample: ExampleWrapper = (scriptBody, opts) =>
  [
    `blackboxqa-browser${opts?.connect === true ? " --connect" : ""} <<'EOF'`,
    scriptBody,
    "EOF",
  ].join("\n");

// blackboxqa: every script runs as one named step inside a session.
export const sessionExample: ExampleWrapper = (scriptBody, opts) =>
  [
    `blackboxqa run --session "$id" --step ${opts?.step ?? "<name>"} <<'EOF'`,
    scriptBody,
    "EOF",
  ].join("\n");

export interface ScriptingGuideOptions {
  // Include blackboxqa-browser-only material (PowerShell --connect piping, the
  // "Connecting to a running Chrome instance" section, --browser/--connect/
  // --headless tips). Must stay out of blackboxqa's help. Default false.
  browserExtras?: boolean;
  // How examples are invoked for this CLI.
  example: ExampleWrapper;
  // Section heading: "LLM USAGE GUIDE:" (blackboxqa-browser) / "SCRIPTING GUIDE:" (blackboxqa).
  heading: string;
}
