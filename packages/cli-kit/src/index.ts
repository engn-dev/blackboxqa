...
// `--json` (or BBQA_LOG_JSON=1) forces machine-readable JSON on stderr,
...
function wantsJsonLogs(): boolean {
  return process.argv.includes("--json") || process.env.BBQA_LOG_JSON === "1";
}

function resolveCliLevel(): LogLevel | undefined {
  if (process.env.BBQA_LOG_LEVEL) {
...
// Root CLI logger shared by the blackboxqa CLIs. Diagnostics go to stderr so stdout
// stays clean for machine-readable output. Quiet by default (warn);
// `--verbose`/`-v` or BBQA_LOG_LEVEL raises it, `--json` forces structured
