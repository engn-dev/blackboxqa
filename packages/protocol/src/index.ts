// BlackboxQA daemon IPC protocol — single source of truth.
//
// All requests and responses on the daemon's named-pipe / Unix socket are
// validated against these Zod schemas. CLIs (`blackboxqa`, `blackboxqa-browser`)
...
// Shaped by the daemon, consumed by the `blackboxqa` orchestrator. Like the other
...
// the same slug. Pure JS (djb2) — no node:crypto — so @blackboxqa/protocol stays
...
// ~/.blackboxqa (installed by `blackboxqa install`). Both the daemon's socket `install`
// handler (apps/blackboxqa-daemon) and the daemon-client extract path write the
