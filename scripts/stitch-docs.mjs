...
//        cli=npx-cli   substitute `{{cli}}` with `npx @blackboxqa/cli`
//                      (default `blackboxqa` — what the CLI help shows)
...
//      snippet (`{{cli}}` pre-substituted with `blackboxqa`), imported by
...
// Markdown surfaces scanned for marker regions. plugins/blackboxqa/skills is a
...
const CLI_SUBSTITUTIONS = {
  blackboxqa: "blackboxqa",
  "npx-cli": "npx @blackboxqa/cli",
};

const OPEN_MARKER =
  /^<!-- blackboxqa:snippet ([a-z0-9-]+)((?: [a-z]+=[a-z0-9-]+)*) -->$/;
const CLOSE_MARKER = /^<!-- blackboxqa:end ([a-z0-9-]+) -->$/;
...
function renderSnippet(snippets, name, flags, context) {
  ...
  const cli = flags.cli ?? "blackboxqa";
...
function generateTs(snippets) {
  ...
  const lines = [
    ...
    "// `{{cli}}` tokens are pre-substituted with the bare `blackboxqa` invocation.",
    "",
  ];
...
for (const target of await markdownTargets()) {
  ...
  if (!source.includes("<!-- blackboxqa:snippet ")) {
    continue;
  }
  await reconcile(
    target,
