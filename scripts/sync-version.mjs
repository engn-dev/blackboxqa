...
// (root + apps/* + packages/*, except @blackboxqa/config which is intentionally
...
// @blackboxqa/config has no meaningful version; leave it at 0.0.0.
const SKIP = new Set(["@blackboxqa/config"]);
...
for (const manifestPath of [
  ...
  join(root, "plugins", "blackboxqa", ".codex-plugin", "plugin.json"),
]) {
  try {
