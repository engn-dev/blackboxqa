...
# BlackboxQA session (recorded QA)

Work the flow like a tester — observe, act, adapt — not as a pre-written script. Every script runs
as a **step** against one persistent browser; BlackboxQA records trace / video / HAR / console and
renders a self-contained `report.html`. Use the **blackboxqa-scripting** skill for the API.
...
- Any run where "what happened?" needs a report (for a quick one-off, use **blackboxqa-automate**).
...
1. Ensure the runtime: `npx @blackboxqa/cli install` (one-time).
2. Start: `id=$(npx @blackboxqa/cli session start --name "<flow>")`
3. **LOOK** — observe before acting; an observe step records like any other:
   ```sh
   npx @blackboxqa/cli run --session "$id" --step observe-home <<'EOF'
...
5. **ACT** — run that one action (or a tight cluster, e.g. fill three fields + submit) as its own
   intent-named step: `npx @blackboxqa/cli run --session "$id" --step submit-login-form <<'EOF' …`
...
8. End + render: `npx @blackboxqa/cli session end "$id"` → `~/.blackboxqa/sessions/<id>/report.html`
9. Offer **blackboxqa-review** (or `npx @blackboxqa/ui`) to browse it.
10. Done? Leave the daemon running for the next session, or `npx @blackboxqa/cli stop` to shut it
...
- Don't invent API shapes; use the blackboxqa-scripting reference.
