---
name: blackboxqa-scripting
description: The BlackboxQA sandbox scripting API for browser automation. Use when writing or debugging a BlackboxQA script — looking up how to open a page, click, fill, extract text, observe an unknown page with snapshotForAI, evaluate in the page, take a screenshot, persist data between steps, or understand sandbox limits (no imports, timeouts). Trigger phrases — "how do I click in blackboxqa", "blackboxqa page API", "what's on this page", "explore a page in blackboxqa", "snapshotForAI", "saveScreenshot signature", "get text from the page", "why is my blackboxqa script timing out", "open a new tab in blackboxqa".
license: MIT
metadata:
  author: blackboxqa
  version: 0.4.4
...
# BlackboxQA scripting API

BlackboxQA scripts are plain **async JavaScript** run in a QuickJS sandbox with a Playwright-like API.
Both `blackboxqa-browser run` (one-off) and `blackboxqa run --session` (recorded step) execute the same way:
top-level `await`, with `browser`, `console`, and the file helpers available as globals.

## When to use

- Writing a script to drive a browser with BlackboxQA
...
User says: "navigate to a site and get the title in blackboxqa" or "how do I read text off the page?"
...
<!-- blackboxqa:snippet ex-snapshot fenced=js -->
```js
...
```
<!-- blackboxqa:end ex-snapshot -->

<!-- blackboxqa:snippet api-snapshot -->
...
<!-- blackboxqa:snippet api-globals -->
...
<!-- blackboxqa:end api-globals -->

<!-- blackboxqa:snippet rule-data-passing -->
...
<!-- blackboxqa:end rule-data-passing -->
