...
- For a recorded, verifiable run **with** a report, use **blackboxqa-session** instead.
...
1. If the runtime isn't installed: `npx @blackboxqa/cli install` (one-time; downloads Chromium).
2. Write a short, focused script with the blackboxqa-scripting API (`browser.getPage`, `page.goto`,
   `locator`/`evaluate`, `console.log` the result), observing first on unknown pages (see *Hard
   rules*).
3. Run it: `npx @blackboxqa/browser run ./script.js` (or pipe the script via stdin).
...
6. Cleanup (optional): the run leaves a shared background daemon up for reuse. To shut it (and any
   browser) down, run `npx @blackboxqa/browser stop` (alias of `blackboxqa stop` / `blackboxqa daemon stop`).
