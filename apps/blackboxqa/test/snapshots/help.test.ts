...
// `blackboxqa --help` and `blackboxqa run --help` must be fully self-contained for an
...
// @blackboxqa/cli-kit), with no dead-end pointer to the separately-installed
// `blackboxqa-browser` binary and no engine-only flags leaking in.
describe("--help content", () => {
  ...
  it("root --help has no dead-end pointer and no engine-only flags", async () => {
    ...
    // The old indirection ("see `blackboxqa-browser --help` for the full
    // reference") must be gone — blackboxqa-browser may not even be installed.
    // (The additive "use `blackboxqa-browser run` for one-offs" tip is fine.)
    expect(out.stdout).not.toContain("see `blackboxqa-browser --help`");
    expect(out.stdout).not.toContain("blackboxqa-browser --help");
  ...
  it("`run --help` carries the full scripting reference and step semantics", async () => {
    ...
    expect(out.stdout).not.toContain("blackboxqa-browser");
    expect(out.stdout).not.toContain("--connect");
  });
});
