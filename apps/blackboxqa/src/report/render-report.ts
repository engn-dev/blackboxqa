import { formatDurationMs } from "@blackboxqa/cli-kit";
import { sessionStepSlug } from "@blackboxqa/protocol";
...
function renderHeader(m: SessionManifest): string {
  return `
  <header class="rhead">
    <div class="crumb">BlackboxQA <span class="sep">›</span> ${escapeHtml(m.id)}</div>
...
<title>BlackboxQA report — ${escapeHtml(manifest.name ?? manifest.id)}</title>
<style>${STYLE}</style>
</head>
<body>
<div class="page">
