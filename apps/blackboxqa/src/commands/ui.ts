...
import { sessionsRootDir } from "@blackboxqa/daemon-client";
...
// Launch the @blackboxqa/ui web app in the FOREGROUND (unlike the detached daemon):
...
export async function uiCommand(args: UiArgs): Promise<number> {
  ...
  if (!resolved) {
    process.stderr.write(
      "blackboxqa ui: could not locate the @blackboxqa/ui app. Build it with `pnpm --filter @blackboxqa/ui build`, or set BBQA_UI_SERVER.\n"
    );
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("blackboxqa ui: could not allocate a port.\n");
    return 1;
  }

  const url = `http://${host}:${port}`;
  const baseEnv = { ...process.env, BBQA_UI_ROOT: root };
  ...
  if (resolved.kind === "standalone") {
    ...
    process.stderr.write(
      "blackboxqa ui: no production build found; starting the dev server (slower first paint). Run `pnpm --filter @blackboxqa/ui build` for fast startup.\n"
    );
    child = spawn(
      process.execPath,
  ...
  if (!ready) {
    ...
    await exited;
    process.stderr.write(
      "blackboxqa ui: server did not become ready in time.\n"
    );
    return 1;
  }
  ...
  if (args.json) {
    ...
    process.stdout.write(
      `blackboxqa ui listening on ${url}\n  source: ${root}\n  Press Ctrl-C to stop.\n`
    );
  }
  if (args.open) {
    openBrowser(url);
