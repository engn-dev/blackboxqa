import { requestId } from "@blackboxqa/cli-kit";
import {
  currentDaemonPid,
  isDaemonRunning,
  sendRequest,
  waitForDaemonExit,
} from "@blackboxqa/daemon-client";
import type { StatusSummary } from "@blackboxqa/protocol";
...
// `blackboxqa daemon stop` — first-class, graceful daemon shutdown that mirrors
// `blackboxqa-browser stop`. Reuses the daemon's existing "stop" RPC.
...
export async function stopDaemonIfIdle(
  ...
  if (others > 0 || browsers > 0) {
    if (!json) {
      process.stdout.write(
        `Daemon left running — ${others} other session(s), ${browsers} browser(s) active. Run \`blackboxqa daemon stop\` to stop it.\n`
      );
    }
    return;
  }
