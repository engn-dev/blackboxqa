...
// How `blackboxqa ui` should launch the @blackboxqa/ui web app:
...
// Walk up from this module's location looking for the apps/blackboxqa-ui workspace.
// Works whether the CLI runs bundled (apps/blackboxqa/dist/cli.js) or via tsx
// (apps/blackboxqa/src/cli.ts) — both sit under the monorepo root.
function findWorkspaceUiDir(): string | null {
  let dir = path.dirname(fileURLToPath(import.meta.url));
  let parent = path.dirname(dir);
  while (dir !== parent) {
    const ui = path.join(dir, "apps", "blackboxqa-ui");
...
export async function resolveUiServer(): Promise<ResolvedUiServer | null> {
  ...
  const override = process.env.BBQA_UI_SERVER;
