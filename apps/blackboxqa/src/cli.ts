import { isMainModule } from "@blackboxqa/cli-kit";
...
const VERSION = process.env.BBQA_CLI_VERSION ?? "0.0.0-dev";
...
export function buildProgram(): CommandType {
  ...
  program
    ...
    .option(
      "--dir <PATH>",
      "Source folder to open (default: ~/.blackboxqa/sessions)"
    )
    .option(
      "--port <PORT>",
  ...
  program
    .command("init")
    .description(
      "Set up blackboxqa: install the runtime, then print next steps"
    )
