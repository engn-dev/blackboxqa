...
describe("roots registry (BBQA_DIR-isolated)", () => {
  let home: string;
  beforeEach(async () => {
    home = await mkdtemp(path.join(os.tmpdir(), "blackboxqa-home-"));
    process.env.BBQA_DIR = home;
  });
  afterEach(async () => {
    Reflect.deleteProperty(process.env, "BBQA_DIR");
    Reflect.deleteProperty(process.env, "BBQA_UI_ROOT");
  ...
  it("selects BBQA_UI_ROOT (the --dir launch root) by default", async () => {
    process.env.BBQA_UI_ROOT = "/tmp/launch-root";
    const { lastRootId, roots } = await loadRoots();
