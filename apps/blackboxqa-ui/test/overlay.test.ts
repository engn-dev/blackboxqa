...
describe("withOverlay persistence", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(os.tmpdir(), "blackboxqa-ov-"));
  });
  afterEach(async () => {
    await rm(dir, { force: true, recursive: true });
  });

  it("persists mutations to <root>/.blackboxqa-ui.json", async () => {
    ...
    const raw = JSON.parse(
      await readFile(path.join(dir, ".blackboxqa-ui.json"), "utf8")
    );
