#!/usr/bin/env node
// The BlackboxQA session viewer, runnable straight from npm — the trace-viewer
// pattern, for blackboxqa:
//
//   npx @blackboxqa/ui                    # browse ~/.blackboxqa/sessions
//   npx @blackboxqa/ui --dir ./artifacts  # browse a specific folder
//   npx @blackboxqa/ui --no-open          # don't auto-open a browser
//
...
if (!existsSync(serverEntry)) {
  process.stderr.write(
    "blackboxqa viewer: bundled server (dist/server/entry.mjs) not found — this @blackboxqa/ui install looks incomplete.\n"
  );
  process.exit(1);
}
...
const root = path.resolve(
  arg("--dir", path.join(homedir(), ".blackboxqa", "sessions"))
);
...
if (!ready) {
  child.kill("SIGTERM");
  process.stderr.write(
    "blackboxqa viewer: server did not become ready in time.\n"
  );
  process.exit(1);
}

process.stdout.write(
  `blackboxqa viewer on ${url}\n  source: ${root}\n  Press Ctrl-C to stop.\n`
);
