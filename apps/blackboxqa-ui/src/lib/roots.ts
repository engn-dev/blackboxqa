...
// Guarantee the default ~/.blackboxqa/sessions root is always registered.
...
// Load the registry, seeding the default root and (if `blackboxqa ui --dir` set
// BBQA_UI_ROOT) the launch root, which becomes the selected one. Persists
...
export function loadRoots(): Promise<{
  ...
  return serializeConfig(async () => {
    ...
    const envRoot = process.env.BBQA_UI_ROOT?.trim();
    if (envRoot) {
