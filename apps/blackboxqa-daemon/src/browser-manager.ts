...
import type { BrowserSummary } from "@blackboxqa/protocol";
import {
  type Browser,
  type BrowserContext,
  chromium,
...
export class BrowserManager {
  ...
  constructor(
    baseDir = path.join(os.homedir(), ".blackboxqa", "browsers"),
  ...
  private buildManualConnectError(endpoint: string): string {
    return [
      ...
      "If Chrome is using built-in remote debugging, run `blackboxqa-browser --connect` without a URL so DevToolsActivePort can be auto-discovered.",
