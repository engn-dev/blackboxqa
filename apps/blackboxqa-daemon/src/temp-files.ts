...
import { getBlackboxqaBaseDir } from "./local-endpoint.js";

const SAFE_PATH_SEGMENT_PATTERN = /[^A-Za-z0-9._-]/g;
const NOFOLLOW_FLAG = constants.O_NOFOLLOW ?? 0;

export const BLACKBOXQA_BASE_DIR = getBlackboxqaBaseDir();
export const BLACKBOXQA_TMP_DIR = path.join(BLACKBOXQA_BASE_DIR, "tmp");
...
export async function ensureBlackboxqaTempDir(): Promise<string> {
  await mkdir(BLACKBOXQA_BASE_DIR, {
    recursive: true,
  });
  await assertControlledDirectory(
    BLACKBOXQA_BASE_DIR,
    "BlackboxQA Browser base directory"
  );

  await mkdir(BLACKBOXQA_TMP_DIR, {
    recursive: true,
  });
  await assertControlledDirectory(
    BLACKBOXQA_TMP_DIR,
    "BlackboxQA Browser temp directory"
  );

  return path.resolve(BLACKBOXQA_TMP_DIR);
}

export async function resolveBlackboxqaTempPath(
  fileName: unknown,
  options: {
    createParents?: boolean;
  } = {}
): Promise<string> {
  const rootDir = await ensureBlackboxqaTempDir();
...
export async function writeBlackboxqaTempFile(
  fileName: unknown,
  data: string | Uint8Array
): Promise<string> {
  const destinationPath = await resolveBlackboxqaTempPath(fileName, {
    createParents: true,
  });
...
export async function readBlackboxqaTempFile(
  fileName: unknown
): Promise<string> {
  const destinationPath = await resolveBlackboxqaTempPath(fileName);
  await assertDestinationIsNotSymlink(destinationPath);
