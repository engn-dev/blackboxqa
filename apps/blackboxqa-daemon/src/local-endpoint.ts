...
export function getBlackboxqaBaseDir(homedir = os.homedir()): string {
  return path.join(homedir, ".blackboxqa");
}
...
export function getDaemonEndpoint(
  ...
  if (platform === "win32") {
    ...
    return `\\\\.\\pipe\\blackboxqa-daemon-${username}`;
  }

  return path.join(getBlackboxqaBaseDir(homedir), "daemon.sock");
}

export function getPidPath(homedir = os.homedir()): string {
  return path.join(getBlackboxqaBaseDir(homedir), "daemon.pid");
}

export function getBrowsersDir(homedir = os.homedir()): string {
  return path.join(getBlackboxqaBaseDir(homedir), "browsers");
}

export function getSessionsDir(homedir = os.homedir()): string {
  return path.join(getBlackboxqaBaseDir(homedir), "sessions");
}
