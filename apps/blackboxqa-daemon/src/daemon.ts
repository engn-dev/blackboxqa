...
import { createLogger } from "@blackboxqa/logger";
import {
...
import { ensureBlackboxqaTempDir } from "./temp-files.js";

const BASE_DIR = getBlackboxqaBaseDir();
  ...
  if (targetSession) {
    ...
    if (!sessions.has(targetSession)) {
      await writeMessage(socket, {
        id: request.id,
        type: "error",
        message: `No active session "${targetSession}". Start one with \`blackboxqa session start\`.`,
      });
      return false;
    }
    ...
    case "browser-stop":
      ...
      if (sessions.isSessionBrowser(request.browser)) {
        await writeMessage(socket, {
          id: request.id,
          type: "error",
          message: `"${request.browser}" is a session browser; tear it down with \`blackboxqa session end\` or \`blackboxqa session abort\`.`,
        });
        return;
      }
