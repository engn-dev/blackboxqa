...
import {
  BLACKBOXQA_TMP_DIR,
  ensureBlackboxqaTempDir,
  resolveBlackboxqaTempPath,
} from "../../temp-files.js";
...
describe.sequential("QuickJS sandbox file I/O", () => {
  ...
  beforeAll(async () => {
    ...
    browserRootDir = await mkdtemp(
      path.join(os.tmpdir(), "blackboxqa-quickjs-file-io-")
    );
  ...
  it("saves screenshot buffers into the controlled temp directory", async () => {
    ...
    const expectedPath = await resolveBlackboxqaTempPath(requestedName);
    ...
    expect(
      result.savedPath.startsWith(
        `${path.resolve(BLACKBOXQA_TMP_DIR)}${path.sep}`
      )
    ).toBe(true);
  ...
  it("rejects symlinked temp-file targets", async () => {
    ...
    const symlinkPath = path.join(BLACKBOXQA_TMP_DIR, symlinkName);
