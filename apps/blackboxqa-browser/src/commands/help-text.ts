// Help prose for the `blackboxqa-browser` engine CLI. The sandbox rules, script
// API, and LLM usage guide come from @blackboxqa/cli-kit — the single source of
// truth shared with the `blackboxqa` orchestrator — so the two CLIs' help cannot
...
const INVOCATION_STYLES = `Primary invocation styles:
  ...
  blackboxqa-browser --browser my-project < script.js
...
export const CLI_LONG_ABOUT = `BlackboxQA Browser is a CLI for controlling local or external browsers with JavaScript scripts.
...
export const RUN_LONG_ABOUT =
  ...
  "Use top-level flags before `run`, for example `blackboxqa-browser --browser my-project run script.js`.";
