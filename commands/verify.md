---
description: Turn a code change into a prioritized browser-QA plan with BlackboxQA, then optionally record the flows.
...
to record — for approved ones, record a BlackboxQA session (`/blackboxqa:session` mechanics) and report each
`report.html` with a one-line pass/fail. Offer `/blackboxqa:review` to open it. Already know the flow? Use
`/blackboxqa:session`. Just a quick one-off with no plan? Use `/blackboxqa:run`.
